...
export class CourseService {
  ...
  getUsersCourses(courseState: string): Observable<Course[]> {
    if (courseState === '')
      ...
      return this.http.get<Course[]>(
        `${environment.apiUrl}/api/v1/courses/me?courseState=${courseState}`
      );
  }

  completeCourse(courseId: number): Observable<any> {
    return this.http.put<any>(
      `${environment.apiUrl}/api/v1/completed-courses/${courseId}/complete`,
      {}
    );
  }
  ...
  getUsersCompletedCourses(): Observable<Course[]> {
    return this.http.get<Course[]>(
      `${environment.apiUrl}/api/v1/completed-courses`
    );
  }

  downloadCertificate(courseId: number): Observable<Blob> {
    return this.http.get(
      `${environment.apiUrl}/api/v1/completed-courses/${courseId}/certificate`,
      {
        responseType: 'blob',
      }
    );
